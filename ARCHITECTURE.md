...
- OAuth 2.0 (`video.publish`), токены — зашифрованным файлом вне репозитория (`apps/web/.data/`, AES-256-GCM, ключ из `TOKEN_ENCRYPTION_KEY`), access_token обновляется через refresh_token заранее (за 5 минут до истечения), не по факту 401
- Content Posting API: `video/init/` (chunked `FILE_UPLOAD`) → заливка по `upload_url` → поллинг `status/fetch/`
- **Всегда `privacy_level: SELF_ONLY`** — приложение не проходило app review, публичная публикация недоступна и не реализовывалась; в UI везде формулировки про "черновик", не "опубликовано"
...
- **TikTok Content Posting API** пока используется только в режиме черновика (`SELF_ONLY`) — приложение не проходило app review. Прежде чем менять privacy_level или добавлять публичную публикацию, нужно отдельное согласование (и пройденный review на стороне TikTok).
